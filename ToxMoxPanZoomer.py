...
def update_pan_and_zoom():
    ...
    # Get dimensions
    S_w_native = source_settings["source_base_width"]   # Native width of full source
    S_h_native = source_settings["source_base_height"]  # Native height of full source

    # Get crop values
    crop_left = source_settings["crop_left"]
    crop_top = source_settings["crop_top"]
    crop_right = source_settings["crop_right"]
    crop_bottom = source_settings["crop_bottom"]
    ...
    # Scale factors
    base_scale_x = source_settings["initial_scale_x"] 
    base_scale_y = source_settings["initial_scale_y"]
    ...
    if settings["pan_enabled"]:
        ...
        if source_settings["is_initial_state_captured"]:
            new_pos_x = source_settings["initial_pos_x"]
            new_pos_y = source_settings["initial_pos_y"]
        else: 
    ...
    try:
        if isinstance(scene_item, dict) and scene_item.get("is_direct_source"):
            ...
            source = settings.get("direct_source_cache")
            if source:
...
# Helper function to get monitor information for a specific config
# ... (rest of the script) ...